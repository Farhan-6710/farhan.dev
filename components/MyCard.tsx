...
const MyCard = () => {
  // State to store the current time, and to check if the component is mounted
  const [currentTime, setCurrentTime] = useState<string | null>(null);

  useEffect(() => {
    // Set up the time update only on the client side
    const interval = setInterval(() => {
      setCurrentTime(new Date().toLocaleTimeString());
    }, 1000);

    // Set initial time after mount (just for initial render)
    setCurrentTime(new Date().toLocaleTimeString());
